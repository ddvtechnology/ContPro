...
const tourSteps: TourStep[] = [
  ...
  // ============================================================================
  {
    ...
    description: 'Vou te mostrar rapidamente como usar todas as funcionalidades. O tour tem 21 passos e leva apenas 5 minutos. Vamos começar!',
    position: 'center',
    route: '/',
    section: 'Início',
  },
  ...
  // ============================================================================
  // SEÇÃO 4: CONTROLE DE ITENS DOS CONTRATOS
  // ============================================================================
  {
    target: 'a[href="/contract-items"]',
    title: 'Controle de Itens dos Contratos 📦',
    description: 'Gerencie o estoque de itens vinculados a cada contrato! Importe planilhas com centenas de itens, registre pedidos que abatam automaticamente do estoque e monitore alertas de estoque baixo. Tudo em um só lugar!',
    position: 'right',
    route: '/',
    section: 'Controle de Itens',
  },
  {
    target: 'CONTRACT_SEARCH_SELECT',
    title: 'Seleção de Contrato com Busca',
    description: 'Use este campo para selecionar o contrato. Digite parte do número ou nome do contratado para buscar rapidamente. Cada contrato pode ter sua própria lista de itens e pedidos!',
    position: 'bottom',
    route: '/contract-items',
    section: 'Controle de Itens',
  },
  {
    target: 'BUTTON_IMPORTAR_ITENS',
    title: 'Importação em Lote de Itens',
    description: 'Clique aqui para importar múltiplos itens de uma vez via Excel! Baixe o template, preencha com seus itens (nome, quantidade, unidade, estoque mínimo) e faça upload. Muito mais rápido que cadastrar um por um!',
    position: 'bottom',
    route: '/contract-items',
    section: 'Controle de Itens',
  },
  {
    target: 'BUTTON_IMPORTAR_PEDIDOS',
    title: 'Importação em Lote de Pedidos',
    description: 'Registre dezenas de pedidos de uma vez! O template mostra os itens disponíveis no contrato. Ao confirmar, o estoque é abatido automaticamente. Ideal para registros diários ou semanais!',
    position: 'bottom',
    route: '/contract-items',
    section: 'Controle de Itens',
  },
  {
    target: 'TABLE_ITENS',
    title: 'Tabela de Itens e Estoque',
    description: 'Veja todos os itens do contrato com status visual: Verde (OK), Amarelo (Baixo), Vermelho (Esgotado). Use checkboxes para selecionar múltiplos itens e excluir em lote. Ações: 🛒 = Registrar pedido | Editar | Excluir.',
    position: 'top',
    route: '/contract-items',
    section: 'Controle de Itens',
  },
  {
    target: 'CHECKBOX_SELECT_ALL',
    title: 'Seleção Múltipla',
    description: 'Clique no checkbox do cabeçalho para selecionar todos os itens, ou selecione individualmente. Itens selecionados ficam com fundo azul. Use o botão "Excluir (N)" que aparece para deletar múltiplos itens de uma vez!',
    position: 'right',
    route: '/contract-items',
    section: 'Controle de Itens',
  },
  ...
  // ============================================================================
  // SEÇÃO 5: DOCUMENTOS E SEUS ELEMENTOS
  // ============================================================================
  {
  ...
  // ============================================================================
  // SEÇÃO 6: RELATÓRIOS E SEUS ELEMENTOS
  // ============================================================================
  {
  ...
  // ============================================================================
  // SEÇÃO 7: NOTIFICAÇÕES E SUAS OBSERVAÇÕES
  // ============================================================================
  {
  ...
  // ============================================================================
  // SEÇÃO 8: MENU DO USUÁRIO E SEUS ELEMENTOS
  // ============================================================================
  {
  ...
  useEffect(() => {
    ...
    const findSpecialElement = (target: string): HTMLElement | null => {
      switch (target) {
        ...
        case 'CONTRACT_SEARCH_SELECT':
          // Buscar pelo label "Contrato" e pegar o campo de seleção abaixo
          const labels = Array.from(document.querySelectorAll('label'));
          const contratoLabel = labels.find(l => l.textContent?.trim() === 'Contrato');
          if (contratoLabel && contratoLabel.nextElementSibling) {
            return contratoLabel.nextElementSibling as HTMLElement;
          }
          return null;
        case 'BUTTON_IMPORTAR_ITENS':
          return findElementByText('Importar Itens', 'button');
        case 'BUTTON_IMPORTAR_PEDIDOS':
          return findElementByText('Importar Pedidos', 'button');
        case 'TABLE_ITENS':
          // Buscar pela tabela na página de itens
          const tablesItens = document.querySelectorAll('table');
          for (const table of tablesItens) {
            // Verificar se a tabela tem as colunas de itens
            const hasItemColumns = table.textContent?.includes('Item') && 
                                   table.textContent?.includes('Unidade') &&
                                   table.textContent?.includes('Qtd. Atual');
            if (hasItemColumns && table.offsetParent !== null) {
              return table as HTMLElement;
            }
          }
          return null;
        case 'CHECKBOX_SELECT_ALL':
          // Buscar pelo primeiro checkbox na thead da tabela
          const tableHead = document.querySelector('thead');
          if (tableHead) {
            const firstButton = tableHead.querySelector('button');
            if (firstButton) {
    ...
    const checkElement = () => {
      ...
      if (step.target.startsWith('BUTTON_') || 
          ...
          step.target === 'TABLE_ITENS' ||
          step.target === 'DASHBOARD_CARDS' ||
          step.target === 'REPORTS_FILTERS' ||
          step.target === 'CONTRACT_SEARCH_SELECT' ||
          step.target === 'CHECKBOX_SELECT_ALL') {
