...
export default function ImportModal({ contractId, contractNumber, onClose, onSuccess }: ImportModalProps) {
  ...
  const downloadTemplate = () => {
    ...
    ws['!cols'] = [
      ...
      { wch: 40 }, // descricao
      { wch: 12 }, // unidade
      { wch: 18 }, // quantidade_inicial
      { wch: 16 }, // estoque_minimo
      { wch: 16 }  // valor_unitario
    ];

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Itens');
    
    XLSX.writeFile(wb, `template_itens_contrato_${contractNumber}.xlsx`);
  };
  ...
  const parseFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const validatedItems = jsonData.map((row, index) => {
          const errors: string[] = [];
          
          // Converter valores numéricos
          const quantidadeInicial = parseNumericValue(row.quantidade_inicial);
          const estoqueMinimo = parseNumericValue(row.estoque_minimo);
          const valorUnitario = parseNumericValue(row.valor_unitario);
          
          if (!row.nome || String(row.nome).trim() === '') {
            errors.push('Nome é obrigatório');
          }
          
          if (!row.unidade || String(row.unidade).trim() === '') {
